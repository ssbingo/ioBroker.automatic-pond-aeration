...
/**
 * Whether the pump should be running right now to follow the aeration demand: on while the
 * interlock is clear AND at least `minOpenValves` valves are open, off otherwise. The dead-head
 * interlock (evaluateSafety) is the emergency stop on top of this; anti short-cycle (canSwitchPump)
 * gates the actual switch. Pure so the on-path can be unit-tested directly.
 *
 * @param {boolean} interlockActive - whether the dead-head interlock is currently tripped
 * @param {number} openValveCount - number of aeration valves currently open
 * @param {number} minOpenValves - the minimum valves that must be open while the pump runs
 * @returns {boolean} the desired pump state
 */
function pumpShouldRun(interlockActive, openValveCount, minOpenValves) {
	const minOpen = Math.max(1, Number(minOpenValves) || 1);
	return !interlockActive && Number(openValveCount) >= minOpen;
}
