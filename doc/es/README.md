...
> ⚠️ **Estado del proyecto: estructura inicial / trabajo en curso.** Esta versión establece el
> esqueleto del adaptador (ciclo de vida, objetos base, modelo de configuración y la base del
> enclavamiento de seguridad). El motor de control, los backends de hardware y las funciones de
...
| `control.enabled` | boolean (escribible) | `switch.enable` | Habilitación principal (comando) |
| `safety.interlockActive` | boolean | `indicator.alarm` | Enclavamiento de seguridad actualmente activo |

Se irán añadiendo más puntos de datos (por punto de aireación, grupos, sensores, seguridad y
estadísticas) a medida que se implementen las funciones correspondientes; cada nuevo estado se
documentará aquí.
