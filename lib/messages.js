...
const MESSAGES = {
	...
	buttonOverrideOn: {
		en: 'Override button pressed on "{point}" — forcing it on (priority over the automatic control).',
		de: 'Taster an „{point}" gedrückt — wird zwangsweise eingeschaltet (Vorrang vor der Automatik).',
		ru: 'Нажата кнопка ручного управления на «{point}» — принудительно включено (приоритет над автоматикой).',
		pt: 'Botão de substituição pressionado em "{point}" — a forçar ligado (prioridade sobre o controlo automático).',
		nl: 'Handmatige knop ingedrukt op "{point}" — wordt geforceerd ingeschakeld (voorrang op de automatiek).',
		fr: 'Bouton de dérogation pressé sur « {point} » — forcé en marche (priorité sur la commande automatique).',
		it: 'Pulsante di override premuto su "{point}" — forzato acceso (priorità sul controllo automatico).',
		es: 'Botón de anulación pulsado en "{point}" — forzado encendido (prioridad sobre el control automático).',
		pl: 'Naciśnięto przycisk ręczny na „{point}" — wymuszono włączenie (priorytet nad automatyką).',
		uk: 'Натиснуто кнопку ручного керування на «{point}» — примусово увімкнено (пріоритет над автоматикою).',
		'zh-cn': '“{point}”上的手动按钮已按下——强制开启（优先于自动控制）。',
	},
	buttonOverrideOff: {
		en: 'Override button released on "{point}" — back to automatic control.',
		de: 'Taster an „{point}" aufgehoben — zurück zur Automatik.',
		ru: 'Ручное управление на «{point}» снято — возврат к автоматике.',
		pt: 'Substituição desativada em "{point}" — de volta ao controlo automático.',
		nl: 'Handmatige overname op "{point}" opgeheven — terug naar de automatiek.',
		fr: 'Dérogation annulée sur « {point} » — retour à la commande automatique.',
		it: 'Override disattivato su "{point}" — ritorno al controllo automatico.',
		es: 'Anulación desactivada en "{point}" — de vuelta al control automático.',
		pl: 'Wyłączono tryb ręczny na „{point}" — powrót do automatyki.',
		uk: 'Ручне керування на «{point}» скасовано — повернення до автоматики.',
		'zh-cn': '“{point}”上的手动控制已解除——恢复自动控制。',
	},
};
