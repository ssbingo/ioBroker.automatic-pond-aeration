...
| **M7** | ESP32-Backend + Firmware | HTTP/WS-Backend, Discovery, Heartbeat/Failsafe; Referenz-Firmware für **Waveshare ESP32-S3-POE-ETH-8DI-8RO** (8 Relais via TCA9554, 8 DI, PoE-Ethernet) — **hardwaregenau nach [Anhang A](#anhang-a--zielhardware-m7-waveshare-esp32-s3-poe-eth-8di-8ro)**; **Referenzsensoren O₂/Druck/Wassertemp am ESP** (siehe `dev/hardware/sensors.md`) inkl. Sensor-Quellen-/Typ-Modell; **je Kanal ein Taster an den 8 DI** (Übersteuerung, `lib/control/button.js`, backend-agnostisch bereits in v0.0.16); **Verdrahtung/Leistung 24 V DC Ventile + 230 V AC Pumpe** (siehe `dev/hardware/wiring.md`); **mobile Weboberfläche direkt vom ESP (zwingend Port 80)** | ESP32 direkt steuerbar, Boot-/Netz-Failsafe getestet; Taster + Weboberfläche per Handy erreichbar |
