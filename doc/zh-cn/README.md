...
* 直接的 **ESP32** 硬件后端 + 参考固件（Waveshare ESP32-S3-POE-ETH-8DI-8RO），并包含连接到 ESP32 的参考传感器（溶解氧、
  气路压力、水温）——参见 [dev/hardware/sensors.md](../../dev/hardware/sensors.md)；
* 一个**由 ESP32 直接提供的移动端友好网页（强制运行在端口 80 上）**，用于通过手机在现场进行控制和监测——无需 ioBroker 即可操作；
