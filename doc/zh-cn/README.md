...
- **时间计划** – 在按星期几设置的时间窗口内开启选定的点/组（`从`/`到`，例如 `08:00`–`18:00`；支持像 `22:00`–`06:00` 这样跨夜的窗口）。生效的时间计划**优先于循环轮询 / 序列**。
- **冬季 / 防结冰模式** – 在配置的季节内（**开始**/**结束**为循环的 `MM-DD` 形式，例如 `11-01`–`03-15`，可跨年）强制开启选定的点，以保持一个不结冰的冰洞。可选勾选**仅在寒冷时（防冻保护）**并设置**气温阈值**，使池塘仅在实际结冰时才增氧（这需要气温监测）。将**保持开启的点**留空则对整个池塘增氧。冬季模式运行于 `auto` 运行模式，且与每个程序一样，仍会让位于安全联锁和 feeder 暂停。
...
### 通知
启用通知并选择一个 **messaging 实例**（任何 `messaging` 类型的适配器，例如 Telegram 或 Pushover）。随后，当安全联锁触发或解除、氧报警升起或恢复，以及压力超出或重新进入其范围时，适配器会发送一条简短的本地化消息。
