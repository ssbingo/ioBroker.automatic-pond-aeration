 ...
 */
const MANAGED_PREFIXES = ['aeration', 'groups', 'sensors', 'feeder', 'winter', 'control.point', 'control.group'];
...
function buildObjectModel(config) {
	...
	model.push(
		state('info.dryRun', {
			name: 'Dry-run active (no hardware is switched)',
			role: 'indicator',
			type: 'boolean',
			def: false,
		}),
	...
	points.forEach((p, i) => {
		...
		if (p.buttonEnabled) {
			model.push(
				state(`${base}.buttonOn`, {
					name: 'Manual override button active',
					role: 'indicator',
					type: 'boolean',
					def: false,
				}),
	...
	if (config.o2Enabled) {
		...
		if (config.o2ControlEnabled) {
			sensorStates.push(
				state('sensors.oxygenBoostActive', {
					name: 'Oxygen boost active (aeration forced on)',
					role: 'indicator',
					type: 'boolean',
					def: false,
				}),
	...
	// --- winter / ice-free mode (only when enabled) ---
	if (config.winterEnabled) {
		model.push(container('winter', 'Winter / ice-free mode'));
		model.push(
			state('winter.active', {
				name: 'Winter mode active (aeration forced on)',
				role: 'indicator',
				type: 'boolean',
				def: false,
			}),
		...
		model.push(
			state('winter.frostActive', {
				name: 'Frost protection engaged',
				role: 'indicator',
				type: 'boolean',
				def: false,
			}),
