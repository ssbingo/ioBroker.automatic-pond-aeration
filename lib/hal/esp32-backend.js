...
class Esp32Backend extends HardwareBackend {
	...
	async init() {
		...
		// Dedicated, FREQUENT heartbeat — independent of the status poll. This keeps the firmware's
		// on-device failsafe disarmed regardless of how long the user's pollIntervalSec is. (Coupling
		// the heartbeat to a 30 s status poll made the device drop into failsafe every ~15 s.)
		const sendHeartbeat = () => this.request('POST', '/api/heartbeat').catch(() => {});
		sendHeartbeat();
		this.heartbeatTimer = this.adapter.setInterval(sendHeartbeat, HEARTBEAT_INTERVAL_MS);
	...
	/**
	 * Read the status, mirror it into the adapter's data points and let the adapter re-run its
	 * safety/monitoring on the fresh state. (The heartbeat runs on its own faster timer — see connect().)
	 *
	 * @returns {Promise<void>}
	 */
	async pollOnce() {
