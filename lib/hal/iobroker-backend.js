...
class IoBrokerBackend extends HardwareBackend {
	...
	getCurrentState() {
		...
		// The pump is only "monitored" when a pump state is mapped.
		return {
			valves,
			pumpRunning: this.pumpRunning,
			pumpMonitored: Boolean(this.config.pumpObjectId),
	...
	buildForeignMap() {
		...
		// In dry-run we never write or read back the actuator states — we simulate them locally
		// so the control engine and statistics behave as if the hardware followed instantly.
		// Sensors are still read so monitoring keeps working.
	...
	async setValve(index, open) {
		...
		if (!p || p.backendType !== 'iobroker') {
			this.adapter.log.debug(`setValve(${index}, ${open}) skipped: point is not an ioBroker-backed valve.`);
			return false;
		}
		// Dry-run simulates the valve locally (even without a mapped state) so the whole control
		// engine and the statistics behave as if the hardware followed instantly.
		if (this.config.dryRun) {
			this.adapter.log.info(
				`[DRY-RUN] would ${open ? 'OPEN' : 'CLOSE'} valve ${index} "${p.name}" (${p.objectId || 'no state mapped'}).`,
			);
	...
	async setEmergency(open) {
		if (this.config.dryRun) {
			this.adapter.log.info(`[DRY-RUN] would ${open ? 'OPEN' : 'CLOSE'} the emergency valve.`);
	...
	async setPump(on) {
		...
		if (this.config.dryRun) {
			this.adapter.log.info(`[DRY-RUN] would switch the pump ${on ? 'ON' : 'OFF'}.`);
			this.pumpRunning = on;
