...
describe('lib/control/arbiter – resolveDesiredValves', () => {
	...
	it('lets a manual override button force its channel open in auto mode', () => {
		expect(resolveDesiredValves(ctx({ buttonOn: [false, true, false] }))).to.deep.equal([false, true, false]);
	});

	it('lets the override button win over a feeder pause (only safety overrides it)', () => {
		expect(resolveDesiredValves(ctx({ buttonOn: [true, false, false], feederForcedOff: [true, false, false] }))).to.deep.equal([true, false, false]);
	});

	it('applies the override button even in mode "off"', () => {
		expect(resolveDesiredValves(ctx({ mode: 'off', buttonOn: [false, false, true] }))).to.deep.equal([false, false, true]);
	});

	it('ignores override buttons when the master switch is off', () => {
		expect(resolveDesiredValves(ctx({ masterEnable: false, buttonOn: [true, true, true] }))).to.deep.equal([false, false, false]);
	});
});
