...
function normalizePoint(raw, index, defaultBackend) {
	...
	return {
		...
		// Optional friendly name for this point's override button, shown on the ESP32 web UI
		// (licensed feature). Falls back to the point name on the device when empty.
		buttonName: typeof p.buttonName === 'string' ? p.buttonName.trim().slice(0, 24) : '',
	};
}
...
function validateConfig(rawConfig) {
	...
	// The point mapping type follows the selected hardware backend: with the ioBroker-state backend,
	// ESP32 relay channels make no sense, so every point is coerced to an ioBroker-state mapping.
	// (The admin also only offers ESP32 channels when the ESP32 backend is selected.)
	if (defaultBackend !== 'esp32') {
		points.forEach(p => {
			p.backendType = 'iobroker';
		});
	}

	// --- reserved ESP32 relay channels (pump + emergency/safety valve) ---
	// Computed here (not only in the final config object) so the button rule below can use them.
	const esp32EmergencyRelay = clampNumber(src.esp32EmergencyRelay, 0, MAX_AERATION_POINTS - 1, 6);
	const esp32PumpRelay = clampNumber(src.esp32PumpRelay, 0, MAX_AERATION_POINTS - 1, 7);
	...
	// A per-point manual override button may ONLY be wired to an aeration valve. If an ESP32
	// point sits on the relay channel that drives the pump or the emergency (safety) valve,
	// its button is force-disabled here — those channels are safety-critical and must never be
	// hand-toggled. The admin UI mirrors this by greying the button controls out for such rows.
	points.forEach(p => {
		if (
			p.buttonEnabled &&
			p.backendType === 'esp32' &&
			(p.espChannel === esp32PumpRelay || p.espChannel === esp32EmergencyRelay)
		) {
			p.buttonEnabled = false;
			const role = p.espChannel === esp32PumpRelay ? 'pump' : 'emergency (safety) valve';
			warnings.push(
				`Aeration point "${p.name}" is on the ESP32 relay channel driving the ${role}; its manual override button was disabled (buttons are only allowed on aeration-valve channels).`,
			);
		}
	});
	...
	const config = {
		...
		// The reference firmware always serves on port 80; keep it configurable only for a reverse proxy.
		esp32Port: clampNumber(src.esp32Port, 1, 65535, 80),
		esp32EmergencyRelay,
		esp32PumpRelay,
		// Autonomous fallback: let the ESP32 run the time schedules on its own (against NTP time)
		// while the adapter heartbeat is lost. Opt-in; the round-robin sequence stays adapter-side.
		esp32AutonomousEnabled: Boolean(src.esp32AutonomousEnabled),
		minOpenValves,
		...
		notifyEvents: Array.isArray(src.notifyEvents)
			...
			: [...NOTIFY_EVENTS],
		// When false (default), the dead-head interlock trip that happens while the feeder pauses the
		// aeration points is treated as normal and NOT notified (a pressure alarm is still reported on
		// its own). Set true to be alerted for that interlock trip during feeding as well.
		notifyInterlockDuringFeeding: Boolean(src.notifyInterlockDuringFeeding),
