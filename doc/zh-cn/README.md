...
> 🛑 **警告 —— 开发状态，动物福利（请阅读）。** 本适配器**仍在积极开发中，尚未通过无人值守使用的验证。**
> 它控制着**活体动物的生命维持系统**——一次故障、错误配置或程序缺陷都可能使增氧停止，并**危及你的鱼类及其他池塘
> 生物的健康与生命**（缺氧、冬季没有不结冰的冰洞、气泵在阀门关闭下憋压运行）。**请勿不加检查地使用：**在任何无人
> 值守运行之前，请在你自己的硬件上于足够长的时间内**密切观察并验证每一项功能**，并保留一套独立、经过验证的
> 增氧/故障保护措施。**使用风险自负。** *（本声明在明确撤销之前持续有效。）*

> ⚠️ **项目状态。** 已完整实现，并可从管理界面进行配置：阀门控制（时间计划、循环轮询 round-robin、分组）、防憋压的
> **安全联锁**、**监测**（溶解氧、气温/水温、带报警的压力）、**天文时间与地理位置**、**feeder 联动**、
> **冬季 / 防结冰模式**、**氧闭环**、通过 **messaging 适配器**发送的**通知**、**运行时长统计**、
> **试运行（dry-run）测试模式**、每个点的**强制按钮**，以及直接的 **ESP32** 硬件后端（通过 HTTP 与参考固件通信——
> 可从[固件刷写页面](https://ssbingo.github.io/pond-aeration-flash/)在浏览器中刷写）。默认后端通过现有的
...
阀门和气泵**既可以**通过**现有的 ioBroker 状态**驱动（来自任何提供这些开关的适配器），**也可以直接在一台专用的
ESP32 控制器上**驱动——该控制器运行参考固件，无需额外的 ioBroker 实例。你可以在**硬件后端**（常规选项卡）下选择
这一点；参见[配置 → 常规](#常规)。
...
- **硬件后端** – `现有 ioBroker 状态`（默认）通过其他适配器的状态驱动你的阀门/气泵。`ESP32（直接）`通过 HTTP 与 Waveshare ESP32-S3-POE-ETH-8DI-8RO 上的参考固件通信。可从[固件刷写页面](https://ssbingo.github.io/pond-aeration-flash/)在浏览器中刷写该固件（Chrome/Edge，无需额外软件），然后设置**主机/IP**，并映射**应急阀继电器**和**气泵继电器**（0–7）；各增氧点使用为每个点设置的继电器通道。适配器会推送一份安全配置和一个心跳（heartbeat），使固件在设备本地的故障保护即使在 ioBroker 宕机时也能保护池塘。设备通过其 **IP** 访问——可经由 **Ethernet/PoE** 或**可选的 WiFi**（在设备自带的『设置（Settings）』页面上启用；WiFi 需要设备的外置天线——参见手册）。在**池塘处没有局域网（LAN）**的情况下，设备可完全通过 WiFi 借助其内置的**配置热点**（`pond-aeration-setup`，强制门户 captive portal，地址为 `http://192.168.4.1/`）完成配置——参见手册。
  - **自主计划（无需 ioBroker 运行）** *(仅 ESP32，可选)* – 启用后，适配器也会把你的时间计划推送到设备；如果连接中断，ESP32 会使用其 NTP 时钟自行继续运行它们（防憋压安全联锁仍然有效）。循环序列仍由适配器负责。
  - **固件兼容性** – 适配器与固件通过**协议版本**（硬性约定）进行匹配，而不是按精确的发布编号。此适配器版本使用**协议 1**并**推荐固件 v1.6.0**（最低 v1.0.0）；管理界面会显示这一点，并链接到相应的发布页。连接时，设备的版本和一个兼容性标志会作为 `info.deviceFirmware` 和 `info.firmwareCompatible` 发布，任何协议不匹配都会写入日志。请参见[手册](../../docs/manual/pond-aeration-manual.en.pdf) / 固件仓库中的兼容性对照表。
  - **授权（Licensing）** *(仅当你的固件包含可选的授权覆盖层时)* – 设备运行在某个级别：**free**（仅监测）、**community**（继电器控制）或 **pro**（+ 自主独立运行的计划）；无论哪个级别，安全功能（故障保护、应急阀、防憋压联锁、手动按钮）始终有效。新设备会在试用期内以完整功能（**pro**）运行，之后回退到 free，直到在设备的 `/license` 页面输入激活密钥为止。适配器会在 `info.licenseTier` / `info.licenseTrialDaysLeft` / `info.deviceCode` 下显示状态；如果设备**未获得控制授权**，监测仍会继续正常工作，控制则会被跳过（参见 `info.licenseControlBlocked`）。不带该覆盖层的公开固件不受影响。
    *重新刷写说明：* 激活密钥保存在 ESP 上，并会在**通过浏览器安装器重新刷写时被清除**（会开始一段新的试用期）。**设备代码由硬件派生、永不改变**，因此只需**重新输入同一个激活密钥**即可——无需新的密钥。通过设备的**更新（Update）页面**进行固件**更新**（一键在线更新或文件上传）会保留激活状态和所有设置；只有安装器会将其重置。
  - **传感器镜像** – 每次轮询时，适配器还会把你配置的传感器数据点（溶解氧、水温/气温、压力）推送到设备，使它们出现在 **ESP 自带的网页界面**上（标记为 *(ioBroker)*）——即使是仅作为 ioBroker 状态、并未接线到 ESP 的传感器也是如此。物理接线到 ESP 的传感器仍保持优先；推送的值会在几分钟后失效。需要固件 ≥ 1.1.7。
- **轮询间隔（s）** – 多久轮询一次后端状态（例如 `30`）。

### 增氧点
配置的核心。最多可添加 **8** 个点；每个点对应一个阀门。每个点：
- **名称** – 例如 `Pier`、`Deep zone`。在 **ESP32** 后端下，该名称还会**显示在设备自带的网页界面上**（在该点的继电器通道上）——这是一项**授权功能**（从 **community** 级别起）。`Ch 7 = Notventil`（应急阀）和 `Ch 8 = Pumpe`（气泵）为固定标签。参见 [ESP32 网页界面上的名称](#esp32-网页界面上的名称)。
- **已启用** – 将该点纳入控制。
- **后端** – `ioBroker`（一个外部状态）或 `ESP32`（设备上的一个继电器通道）。只有当**硬件后端**（常规选项卡）设为 `ESP32（直接）`时，`ESP32` 选项才会出现。
- **阀门状态 / 通道** – 对于 ioBroker 后端，选择打开阀门的开关状态（通过对象浏览器）。对于 ESP32 后端，从**下拉菜单**中选择**继电器通道**：驱动**气泵**和**应急阀**的通道会显示为*已保留*，已被其他点占用的通道会显示为*使用中*，因此你只能选择空闲的通道。当没有空闲通道时，通过『后端』列将更多的点添加为 **ioBroker 状态**。
- **强制按钮（override）** *(可选)* – 每个点一个物理按钮（例如 ESP32 的数字输入，或任意布尔状态）。它以**切换（toggle）**方式工作：按一次即强制该点**开启，并优先于自动控制**（时间计划/序列/冬季/氧气），甚至优先于 feeder 暂停——*只有主开关或安全跳闸才能将其覆盖*。再按一次即可释放。（计划支持更多按钮模式；该字段已为此预留。）按钮仅适用于**增氧阀**——位于 ESP32 **水泵**或**应急阀**继电器通道上的点无法配置按钮（该选项会置灰）。在 ESP32 后端下，在**设备上**按下的按钮会回传到 ioBroker（`aeration.point.<n>.buttonOn`）并获得相同的优先级。
- **按钮名称** *(ESP32 后端，可选)* – 该点强制按钮的友好名称，显示在设备网页界面上（见下文）。留空 → 按钮显示该点的名称。

#### ESP32 网页界面上的名称

*(ESP32 后端，**授权功能** —— 从 **community** 级别起可用。)* 为你的通道和按钮设置友好名称，使它们出现在设备自带的网页上，而不是 `Ch 1…8` / `DI 1…8`：

- 适配器会将**每个增氧点的名称**推送到其继电器通道（Ch 1–6），并将每个点可选的**按钮名称**推送到对应的数字输入（DI 1–8）。
- **Ch 7 = Notventil**（应急阀）和 **Ch 8 = Pumpe**（气泵）为**固定**，无法重命名。
- **独立运行（无适配器）：** 相同的名称也可以**在设备上**通过 *Settings → Namen（Kanäle & Taster）* 编辑，并保存在 ESP（NVS）中；当适配器连接时，会用此处配置的名称覆盖它们。
- 在 **free**（未授权）固件上，这些名称会被忽略，页面显示默认的 `Ch`/`DI` 标签。
...
- **循环轮询（round-robin）** – 轮流切换各个点，每个点开启一段**保持时间**（秒）。
  * **序列（点和组）** – 可选地定义一个**有序的步骤循环**，其中每个步骤针对单个**点或整个组**，并可带有各自的保持时间。这样即可运行例如 *组 1 → 组 3 → 点 1 → …*，并自由**混合**点和组。在管理界面中用上/下箭头重新排序各步骤。将序列留空则回退为对所有点的普通循环轮询。
- **时间计划** – 在按星期几设置的时间窗口内开启选定的点/组。**从**/**到**通过**时钟选择器**选取（小时/分钟，24 小时制；支持像 `22:00`–`06:00` 这样跨夜的窗口）。生效的时间计划**优先于循环轮询 / 序列**。
- **冬季 / 防结冰模式** – 在配置的季节内（**开始**/**结束**通过**日历**选取——仅**日和月**有效，每年循环，例如 11月1日 – 3月15日，可跨年）强制开启选定的点，以保持一个不结冰的冰洞。可选勾选**仅在寒冷时（防冻保护）**并设置**气温阈值**，使池塘仅在实际结冰时才增氧（这需要气温监测）。将**保持开启的点**留空则对整个池塘增氧。冬季模式运行于 `auto` 运行模式，且与每个程序一样，仍会让位于安全联锁和 feeder 暂停。

### 传感器
可选监测。为每个传感器勾选**已启用**并选择**源状态**：
- **溶解氧** – 带一个下限阈值（触发 `sensors.oxygenAlarm`）、一个目标值和一个滞回；**氧饱和度 %** 由水温计算得出。
  * **氧闭环** – 启用后，当氧含量低于下限阈值时，适配器会**强制开启增氧**，并保持开启直到恢复到目标值（未设置目标值时为 `low + hysteresis`）。将**增强的点**留空则对整个池塘进行增强。与冬季模式一样，该闭环运行于 `auto` 模式，并让位于安全和 feeder 暂停。
- **气温/水温**。
...
### 安全
本选项卡上的每个字段都在**管理界面内附有说明**，解释它的作用及其影响——请阅读这些说明，因为这是错误取值影响最大的选项卡。
- **气泵运行时最少开启的阀门数** – 防憋压保护（默认 `1`）。
- **看门狗间隔（s）**和 **先通后断（make-before-break）重叠（s）**。
- **气泵** – 是否**可控**、气泵**信号**，以及防止频繁启停的最短开/关时间。可控时，适配器会**驱动气泵以跟随增氧需求**——只要至少有最少数量的阀门开启，气泵就运行，当池塘空闲或发生憋压跳闸时则关闭（并遵守最短开/关时间）；当*不*可控时，气泵仅被观测，仅由应急阀对其加以保护。*在 **ESP32** 后端下，气泵信号即 **ESP32 继电器通道**——与『常规 → 硬件后端』下设置的完全相同，此处一并显示，使两个选项卡永远不会相互矛盾；在 **ioBroker** 后端下，它是一个 ioBroker 状态。*
- **应急阀** – 其**信号**、是否为**常开**（故障安全）、阀门**类型**（电磁阀或电动球阀），以及电动阀的**行程时间**。*在 ESP32 后端下，该信号同样是 ESP32 应急阀继电器通道（与『常规』一致）。*

### 通知
启用通知并选择一个 **messaging 实例**（任何 `messaging` 类型的适配器，例如 Telegram 或 Pushover），然后**勾选哪些事件**应发送消息：
- **安全联锁** – 当防憋压联锁触发或解除时；
- **氧报警** – 当溶解氧过低或恢复时；
- **压力报警** – 当压力超出或重新进入其范围时。

每个边沿（触发与解除）都会发送一条简短的本地化文本。若未勾选任何事件，则不发送任何消息。
...
| `sensors.oxygenBoostActive` | boolean | `indicator` | 氧闭环正在强制开启增氧（仅在启用闭环时） |
...
**冬季 / 防结冰模式**（仅在启用冬季模式时创建）

| 对象 | 类型 | 角色 | 说明 |
|------|------|------|------|
| `winter.active` | boolean | `indicator` | 冬季模式当前正在强制开启增氧 |
| `winter.frostActive` | boolean | `indicator` | 防冻保护已启用（足够寒冷） |

**统计**

| 对象 | 类型 | 角色 | 说明 |
|------|------|------|------|
| `aeration.point.<n>.runtimeTodaySec` | number | `value` | 点 `<n>` 今日运行时长（秒） |
| `aeration.point.<n>.runtimeTotalH` | number | `value` | 点 `<n>` 的总运行时长（小时） |
...
已完成：配置界面、阀门控制（时间计划/round-robin/分组）、防憋压安全联锁、监测、天文与地理位置、feeder 联动、**冬季 / 防结冰模式**、**氧闭环**、**通知**、**运行时长统计**、**试运行**测试模式，以及带参考固件的直接 **ESP32** 硬件后端（可从[固件刷写页面](https://ssbingo.github.io/pond-aeration-flash/)在浏览器中刷写）。**仍待完成：**
