...
- **硬件后端** – `现有 ioBroker 状态`（默认）通过其他适配器的状态驱动你的阀门/气泵。`ESP32（直接）`通过 HTTP 与 Waveshare ESP32-S3-POE-ETH-8DI-8RO 上的参考固件通信。可从[固件刷写页面](https://ssbingo.github.io/pond-aeration-flash/)在浏览器中刷写该固件（Chrome/Edge，无需额外软件），然后设置**主机/IP**，并映射**应急阀继电器**和**气泵继电器**（0–7）；各增氧点使用为每个点设置的继电器通道。适配器会推送一份安全配置和一个心跳（heartbeat），使固件在设备本地的故障保护即使在 ioBroker 宕机时也能保护池塘。
  ...
  - **固件兼容性** – 适配器与固件通过**协议版本**（硬性约定）进行匹配，而不是按精确的发布编号。此适配器版本使用**协议 1**并**推荐固件 v1.2.2**（最低 v1.0.0）；管理界面会显示这一点，并链接到相应的发布页。连接时，设备的版本和一个兼容性标志会作为 `info.deviceFirmware` 和 `info.firmwareCompatible` 发布，任何协议不匹配都会写入日志。请参见[手册](../../docs/manual/pond-aeration-manual.en.pdf) / 固件仓库中的兼容性对照表。
