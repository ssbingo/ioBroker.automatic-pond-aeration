...
class IoBrokerBackend extends HardwareBackend {
	...
	async setValve(index, open) {
		...
		await this.adapter.setForeignStateAsync(p.objectId, valveCommandValue(open, p.onValue, p.offValue), false);
		return true;
	}
	...
	async handleForeignChange(id, state) {
		...
		if (desc.kind === 'valve' && typeof desc.index === 'number') {
			...
			await this.adapter.setStateAsync('safety.pumpRunning', { val: Boolean(state.val), ack: true });
		} else if (desc.kind === 'emergency') {
			await this.adapter.setStateAsync('safety.emergencyValve', { val: Boolean(state.val), ack: true });
		}
	}
